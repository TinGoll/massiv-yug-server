...
export class PanelSystem extends IteratingSystem {
  ...
  protected async processEntity(
    ...
    try {
      ...
      // Если заданы параметры в шаблоне.
      if (panelCmp.data?.panels[0]) {
        if (panelCmp.data.panels[0].type) {
          panel.type = panelCmp.data.panels[0].type;
        }
        if (panelCmp.data.panels[0].name) {
          panel.name = panelCmp.data.panels[0].name;
        }
      }
